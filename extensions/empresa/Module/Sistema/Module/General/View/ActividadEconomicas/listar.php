...
<h1>Listado de ActividadEconomicas</h1>
<p>Actividades económicas del país.</p>
...
foreach($ActividadEconomicas as &$obj) {
	$row = array();
	foreach($columns as $column => &$name) {
		// si es un archivo
		if(is_array($name) && $name['type']=='file') {
			if($obj->{$column.'_size'})
				$row[] = '<a href="'.$_base.$module_url.$controller.'/d/'.$column.'/'.urlencode($obj->codigo).'"><img src="'.$_base.'/img/icons/16x16/actions/download.png" alt="" /></a>';
			else
				$row[] = '';
		}
	...
	$row[] =
		'<a href="'.$_base.$module_url.$controller.'/editar/'.urlencode($obj->codigo).'" title="Editar"><img src="'.$_base.'/img/icons/16x16/actions/edit.png" alt="" /></a> '.
		'<a href="'.$_base.$module_url.$controller.'/eliminar/'.urlencode($obj->codigo).'" title="Eliminar" onclick="return eliminar(\'ActividadEconomica\', \''.$obj->codigo.'\')"><img src="'.$_base.'/img/icons/16x16/actions/delete.png" alt="" /></a>';
	$data[] = $row;
}
...
$maintainer->setId('ActividadEconomicas');
