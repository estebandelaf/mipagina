...
<p>Listado de clientes de la empresa.</p>
...
foreach($Clientes as &$obj) {
	...
	foreach($columns as $column => &$name) {
		...
		if(is_array($name) && $name['type']=='file') {
			if($obj->{$column.'_size'})
				$row[] = '<a href="'.$_base.$module_url.$controller.'/d/'.$column.'/'.urlencode($obj->rut).'"><img src="'.$_base.'/img/icons/16x16/actions/download.png" alt="" /></a>';
			else
				$row[] = '';
		}
	...
	$row[] =
		'<a href="'.$_base.$module_url.$controller.'/editar/'.urlencode($obj->rut).'" title="Editar"><img src="'.$_base.'/img/icons/16x16/actions/edit.png" alt="" /></a> '.
		'<a href="'.$_base.$module_url.$controller.'/eliminar/'.urlencode($obj->rut).'" title="Eliminar" onclick="return eliminar(\'Cliente\', \''.$obj->rut.'\')"><img src="'.$_base.'/img/icons/16x16/actions/delete.png" alt="" /></a>';
	$data[] = $row;
}
