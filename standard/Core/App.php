...
class App {
	...
	public static function uses ($className, $location) {
		...
		if(strpos($location, '.')!==false) {
			...
			$location = 'Module'.DS.implode(DS.'Module'.DS, $parts).DS.$directory;
		}
		// agregar clase
		self::$_classMap[$className] = str_replace('/', DS, $location);
	}
	...
	public static function load ($className, $buscarSingular = true, $buscarSinS = true) {
		...
		foreach (self::$_paths as $path) {
			$file = $path.DS.$location.DS.$className.'.php';
			if (file_exists($file)) {
				return include $file;
			}
		}
